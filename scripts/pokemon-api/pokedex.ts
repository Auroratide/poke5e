import fs from "node:fs/promises"
import path from "node:path"
import { superconsole } from "../superconsole.ts"

export type Pokedex = {
	name: string,
	region: {
		name: string,
	} | null,
	version_groups: {
		name: string,
	}[],
}

let cache: Record<string, any> | undefined = undefined

const CACHE_FOLDER = path.join(import.meta.dirname, "cache")
const CACHE_FILE = path.join(CACHE_FOLDER, "pokedex.json")

export async function getOnePokedex(id: string): Promise<Pokedex | undefined> {
	superconsole.debug(`Fetching pokedex from cache: ${id}`)

	if (cache == null) {
		cache = JSON.parse(await fs.readFile(CACHE_FILE, { encoding: "utf-8" }))
	}

	return cache?.[id]
}
