...
import { ItemsSrd } from "$lib/srd/items/server"

export const prerender = SRD_ENABLED
export const entries: EntryGenerator = createIdEntryGenerator(ItemsSrd.ids)

export const GET: RequestHandler = async ({ params }) => {
	if (!isEdition(params.edition)) error(404)

	const value = await ItemsSrd.one(params.id, params.edition)
