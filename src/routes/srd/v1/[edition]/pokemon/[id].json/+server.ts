...
import { PokemonSrd } from "$lib/srd/pokemon/server"

export const prerender = SRD_ENABLED
export const entries: EntryGenerator = createIdEntryGenerator(PokemonSrd.ids)

export const GET: RequestHandler = async ({ params }) => {
	if (!isEdition(params.edition)) error(404)

	const value = await PokemonSrd.one(params.id, params.edition)
