...
import type { UserAssets } from "$lib/site/user-assets"
...
export class SupabaseFakemonDataProvider implements FakemonDataProvider {
	...
	private toQuery(fakemon: Omit<Data<Fakemon>["species"], "id">): object {
		return {
			...
			_speed_walking: fakemon.speed.walking ?? 0,
			_speed_climbing: fakemon.speed.climbing ?? 0,
			_speed_swimming: fakemon.speed.swimming ?? 0,
			_speed_flying: fakemon.speed.flying ?? 0,
			_speed_hover: fakemon.speed.hover ?? 0,
			_speed_burrowing: fakemon.speed.burrowing ?? 0,
			_sense_darkvision: fakemon.senses.darkvision ?? 0,
			_sense_blindsight: fakemon.senses.blindsight ?? 0,
			_sense_tremorsense: fakemon.senses.tremorsense ?? 0,
			_sense_truesight: fakemon.senses.truesight ?? 0,
