<script lang="ts">
	...
	$: nameLabel = value.type === "ai" ? m.aiName() : m.artistName()
	$: namePlaceholder = value.type === "ai" ? m.eG()+" Gemini" : m.eG()+" @handle"
	$: linkLabel = value.type === "ai" ? m.aiLink() : m.artistLink()
</script>

<p><strong>{m.sourceOf({name: capitalize(id)})}</strong></p>
...
<TextField name="{id}-href" label="{linkLabel}" bind:value={value.href} placeholder="{m.eG()} https://example.com" />
