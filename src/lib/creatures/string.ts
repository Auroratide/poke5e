...
export const evolutionWithLinks = (baseUrl: string, allPokemon: Pokemon[], allMoves: Move[]) => (type: "pokemon" | "move", id: string) => {
	switch (type) {
	case "pokemon": return `<a href="${baseUrl}/pokemon/${id}">${allPokemon.find((it) => it.id === id)?.name ?? id}</a>`
