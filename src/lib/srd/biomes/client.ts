...
export class BiomesSrdClient {
	...
	all(): Promise<BiomesListJson> {
		return this.customFetch(localizeUrl(resolve("/srd/v1/[edition]/biomes.json", { edition: this.edition })).pathname)
			.then((res) => res.json())
	}

	one(id: string): Promise<BiomeJson | undefined> {
		return this.customFetch(localizeUrl(resolve("/srd/v1/[edition]/biomes/[id].json", { edition: this.edition, id })).pathname)
