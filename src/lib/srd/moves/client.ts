...
export class MovesSrdClient {
	...
	all(): Promise<MovesListJson> {
		return this.customFetch(localizeUrl(resolve("/srd/v1/[edition]/moves.json", { edition: this.edition })).pathname)
			.then((res) => res.json())
	}

	one(id: string): Promise<MoveJson | undefined> {
		return this.customFetch(localizeUrl(resolve("/srd/v1/[edition]/moves/[id].json", { edition: this.edition, id })).pathname)
