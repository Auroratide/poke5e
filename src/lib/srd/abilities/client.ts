...
export class AbilitiesSrdClient {
	...
	all(): Promise<AbilityListJson> {
		return this.customFetch(localizeUrl(resolve("/srd/v1/[edition]/abilities.json", { edition: this.edition })))
			.then((res) => res.json())
	}

	one(id: string): Promise<AbilityJson | undefined> {
		return this.customFetch(localizeUrl(resolve("/srd/v1/[edition]/abilities/[id].json", { edition: this.edition, id })))
