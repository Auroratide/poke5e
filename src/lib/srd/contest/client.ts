...
export class ContestSrdClient {
	...
	all(): Promise<ContestListJson> {
		return this.customFetch(localizeUrl(resolve("/srd/v1/[edition]/contest.json", { edition: this.edition })).pathname)
			.then((res) => res.json())
	}

	one(id: string): Promise<ContestJson | undefined> {
		return this.customFetch(localizeUrl(resolve("/srd/v1/[edition]/contest/[id].json", { edition: this.edition, id })).pathname)
