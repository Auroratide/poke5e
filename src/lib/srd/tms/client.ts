...
export class TmsSrdClient {
	...
	all(): Promise<TmListJson> {
		return this.customFetch(localizeUrl(resolve("/srd/v1/[edition]/tms.json", { edition: this.edition })).pathname)
			.then((res) => res.json())
	}

	one(id: string): Promise<TmJson | undefined> {
		return this.customFetch(localizeUrl(resolve("/srd/v1/[edition]/tms/[id].json", { edition: this.edition, id })).pathname)
